...
function getNextFriday(date) {
  ...
  const dif = fridayNum - weekday;

  if (dif > 0) {
    newDate.setDate(day + dif);
  } else {
    newDate.setDate(day + (7 + dif));
  }
  return newDate;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const dateSt = new Date(dateStart);
  const dateEn = new Date(dateEnd);
  const diff =
    (dateEn.getTime() - dateSt.getTime()) / (1000 * 60 * 60 * 24) + 1;
